...
//    env.getCheckpointConfig.enableExternalizedCheckpoints(ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION)//退出不删除checkpoint
      ...
      val ds: DataStream[AllTotal] = mysqlInfoDate
        ...
        //实现数据的聚合，自定义累加器
        .aggregate(new TotalCount, new TotalReWindow)

       ds.print()
      ds.addSink(new MyFlinkKudu())
...
//***************************************************************************************************************************************
//自定义聚合函数
class TotalCount() extends AggregateFunction[(String,Double), (Long,Double),(Long,Double)] {
  //累加操作
...
class TotalReWindow() extends WindowFunction[(Long,Double), AllTotal, String, TimeWindow] {
