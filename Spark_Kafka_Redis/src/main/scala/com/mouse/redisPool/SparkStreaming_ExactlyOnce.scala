...
import com.mouse.ExactlyOnce.GetLogNew.MyRecord
import com.mouse.ExactlyOnce.{GetLogNew, RedisClient}
...
object SparkStreaming_ExactlyOnce {
  def main(args: Array[String]): Unit = {
    ...
    val offsets: Map[TopicPartition, Long] = fromOffsets
    ...
    stream.foreachRDD {
      rdd =>
        ...
        val result: Array[MyRecord] = GetLogNew.processLogs(rdd)
        ...
        result.foreach {
          record =>
            ...
            //使用set保存当天的uv
            val uv_by_day_key = "uv_" + record.hour
